...
var platformBinaries = map[string]struct {
	...
	"amd64": {
		...
		SHA256: "d9f2451d7cac4ed08872911b0ee1b6c8d5a7bef803cda09411940bcf3c6956b2",
	},
	"arm64": {
		URL:    "https://github.com/coder/portabledesktop/releases/download/" + portableDesktopVersion + "/portabledesktop-linux-arm64",
		SHA256: "5fbe38c44978b3a3e4c05bbfdb32a147d3b991368dd77a2b9055f15ab5b07b50",
	},
}
...
func (p *portableDesktop) VNCConn(ctx context.Context) (net.Conn, error) {
	p.mu.Lock()
	session := p.session
	p.mu.Unlock()
...
func (p *portableDesktop) CursorPosition(ctx context.Context) (int, int, error) {
...
func (p *portableDesktop) ensureBinary() error {
	if p.binPath != "" {
		return nil
	}
	...
	if path, err := exec.LookPath("portabledesktop"); err == nil {
		p.logger.Info(context.Background(), "found portabledesktop in PATH",
			slog.F("path", path),
		)
		p.binPath = path
		return nil
	...
	if info, err := os.Stat(cachedPath); err == nil && !info.IsDir() {
		...
		if info.Mode()&0o100 != 0 {
			p.logger.Info(context.Background(), "using cached portabledesktop binary",
	...
	p.logger.Info(context.Background(), "downloading portabledesktop binary",
		slog.F("url", bin.URL),
	...
	for attempt := range downloadRetries {
		if err := downloadBinary(p.httpDo(), bin.URL, bin.SHA256, cachedPath); err != nil {
			lastErr = err
			p.logger.Warn(context.Background(), "download attempt failed",
		...
		p.binPath = cachedPath
		p.logger.Info(context.Background(), "downloaded portabledesktop binary",
			slog.F("path", cachedPath),
		)
		return nil
	}
...
func downloadBinary(client *http.Client, url, expectedSHA256, destPath string) error {
	if err := os.MkdirAll(filepath.Dir(destPath), 0o700); err != nil {
		return xerrors.Errorf("create cache directory: %w", err)
	}

	resp, err := client.Get(url) //nolint:bodyclose // Body is closed below.
	if err != nil {
