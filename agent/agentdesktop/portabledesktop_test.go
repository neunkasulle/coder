...
func TestPortableDesktop_Screenshot(t *testing.T) {
	...
	rec := &recordedExecer{
		scripts: map[string]string{
			"screenshot": `echo '{"data":"abc123","width":1024,"height":768}'`,
		},
	}
...
func TestPortableDesktop_Screenshot_WithTargetDimensions(t *testing.T) {
	...
	rec := &recordedExecer{
		scripts: map[string]string{
			"screenshot": `echo '{"data":"x","width":800,"height":600}'`,
		},
	}
...
func TestDownloadBinary_Success(t *testing.T) {
	...
	err := downloadBinary(srv.Client(), srv.URL, expectedSHA, destPath)
	require.NoError(t, err)
...
func TestDownloadBinary_ChecksumMismatch(t *testing.T) {
	...
	err := downloadBinary(srv.Client(), srv.URL, wrongSHA, destPath)
	require.Error(t, err)
...
func TestDownloadBinary_HTTPError(t *testing.T) {
	...
	err := downloadBinary(srv.Client(), srv.URL, "irrelevant", destPath)
	require.Error(t, err)
...
func TestEnsureBinary_RetriesOnFailure(t *testing.T) {
	...
	var lastErr error
	for i := range 3 {
		lastErr = downloadBinary(srv.Client(), srv.URL, expectedSHA, destPath)
		if lastErr == nil {
			break
		}
		if i < 2 {
